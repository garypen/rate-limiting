...
use tower_shot::BufferedRateLimitLayer;
...
where
    ...
    S::Future: Send,
{
    // Use a wider bound for the histogram to avoid the ValueOutOfRange panic
    ...
    for _ in 0..total_reqs {
        ...
        tasks.push(tokio::spawn(async move {
            ...
            // (Errors here usually mean the Buffer is full or a Timeout occurred)
    ...
    if success_count > 0 {
        ...
        println!("P95 (Success):   {}µs", hist.value_at_quantile(0.95));
...
where
    S: Service<(), Response = &'static str, Error = tower::BoxError> + Clone + Send + 'static,
    S::Future: Send, // <--- This is the missing piece!
{
    let mut hist = Histogram::<u64>::new_with_bounds(1, 60_000_000, 3).unwrap();
    let start = Instant::now();
    ...
    for _ in 0..total_reqs {
        ...
        tasks.push(tokio::spawn(async move {
            let req_start = Instant::now();
            // This measures the time including the "wait" in the buffer
            let res = local_svc.ready().await.unwrap().call(()).await;
            (res, req_start.elapsed())
        }));
    }

    for task in tasks {
        if let Ok((Ok(_), elapsed)) = task.await {
            hist.record(elapsed.as_micros() as u64).unwrap();
        }
    }
    ...
    println!("P50: {}µs", hist.value_at_quantile(0.5));
...
async fn main() {
    ...
    // 1.b. Buffered Fixed Window Stress
    let fixed = Arc::new(FixedWindow::new(NonZeroUsize::new(limit).unwrap(), period));
    let fixed_svc = BufferedRateLimitLayer::new(fixed, 1_000).layer(service_fn(mock_db_call));
    run_load_test("Buffered Fixed Window", fixed_svc, total_reqs).await;
    ...
    // 2.b. Sliding Window Stress
    let sliding = Arc::new(SlidingWindow::new(
        NonZeroUsize::new(limit).unwrap(),
        period,
    ));
    let sliding_svc = BufferedRateLimitLayer::new(sliding, 1_000).layer(service_fn(mock_db_call));
    run_load_test("Buffered Sliding Window", sliding_svc, total_reqs).await;
    ...
    let bucket_svc = BufferedRateLimitLayer::new(bucket, 1_000).layer(service_fn(mock_db_call));
    run_load_test("Buffered Token Bucket", bucket_svc, total_reqs).await;

    // 3.c. Token Bucket Stress
    // (Assuming TokenBucket::new(capacity, refill_rate_per_sec))
    let bucket = Arc::new(TokenBucket::new(
        NonZeroUsize::new(limit).unwrap(),
        limit,
    ...
    // 4.b. Tower Built-in (Buffered to allow the 5_000 reqs to queue)
